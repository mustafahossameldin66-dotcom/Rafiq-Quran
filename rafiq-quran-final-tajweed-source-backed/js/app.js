...
async function resolvePendingReciter(r){
  ...
  try{const resp=await fetch(MP3QURAN_RECITER_API,{cache:'no-store'});if(!resp.ok)throw new Error();const list=normalizeMp3QuranReciters(await resp.json());const target=String(r.name||'').trim();const found=list.find(x=>x.name===target)||list.find(x=>target.includes(x.name)||x.name.includes(target));if(found){const idx=reciters.findIndex(x=>x.name===target);if(idx>=0)reciters[idx]=found;window.RAFIQ_RECITERS=reciters;try{localStorage.setItem('rafiq-mp3quran-reciters-v1',JSON.stringify(list))}catch{}syncRecitationControl();renderRecitations();updateQuranReciterButton();return found;}}catch{}toast('تعذر ربط هذا القارئ الآن؛ جرّب بعد لحظات.');return r;}
...
function ocean(){
  ['#oceanCanvas','.wind-streams','.light-wind-dust','.wind-ribbons','.rafiq-radiance'].forEach(sel=>document.querySelectorAll(sel).forEach(el=>el.remove()));
  return;
