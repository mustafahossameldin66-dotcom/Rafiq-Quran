...
'use strict';
import { state, DEFAULT, replaceState } from './state.js';
import { loadState, scheduleSave, flushSave, deepMerge } from './storage.js';
import { burstParticles, setOceanBubbles } from './particles.js';
import { surahs, dailyVerses, method, reminders, asbab, wordMeanings, tazkiyah, adhkar } from './data.js';
...
function load(){loadState(state,DEFAULT);if(!state.firstDate&&state.entries.length)state.firstDate=state.entries.map(e=>e.date).sort()[0]||dayKey();save()}
...
function particles(x,y){burstParticles(x,y,state.graphics)}
...
function importJSON(file){const r=new FileReader();r.onload=e=>{try{replaceState(deepMerge(DEFAULT,JSON.parse(e.target.result)));flushSave(state);location.reload()}catch{toast('ملف غير صالح')}};r.readAsText(file)}
...
async function loadQuranBook(){if(quranBook)return quranBook;try{const r=await fetch(QURAN_BASE,{cache:'force-cache'});if(!r.ok)throw new Error('quran file');quranBook=await r.json();return quranBook}catch(e){toast('تعذر تحميل المصحف المحلي');return null}}
...
function createOceanBubbles(){setOceanBubbles(state.graphics)}
function createGlobalOceanBubbles(){setOceanBubbles(state.graphics)}
...
function ritualDayIndex(){const first=state.firstActiveBoundary||dayKey();return Math.max(1,dayDiff(first,dayKey())+1)}
...
/* Final DOM hydration: only depth metadata remains; bubbles are canvas-rendered. */
(function(){
...
// Compatibility bridge for the original inline HTML handlers.
