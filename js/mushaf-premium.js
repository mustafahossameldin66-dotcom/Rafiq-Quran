(function(){
  ...
  async function renderStudyBody(s,v){const box=$('#ayahStudyInner');if(!box)return;if(studyTab==='tajweed'){renderTajweed(box,v);return}loading(box);if(studyTab==='overview'){box.innerHTML=`<div class="ayah-detail-grid"><section class="ayah-detail"><h4>📖 التفسير</h4><p>تفسير هذه الآية من مصادر Quranpedia.</p></section><section class="ayah-detail"><h4>🔎 معاني الكلمات</h4><p>المعاني اللغوية المرتبطة بالآية.</p></section><section class="ayah-detail wahidi-mini"><h4>🕊️ سبب النزول</h4><p>مادة أسباب النزول المرتبطة بهذه الآية، مع مصادر الواحدي المتاحة عبر Quranpedia.</p></section><section class="ayah-detail"><h4>🎙️ التجويد</h4><p>تحليل تعليمي محلي قابل للاستخدام دون اتصال.</p></section></div>${sourceLinks(s.s,v.a)}`;return}
    ...
    if(html){box.innerHTML=`<section class="ayah-detail source-fragment-wrap"><div class="source-kicker">${type==='meanings'?'🔎 معاني الكلمات':'🕊️ أسباب النزول'}</div><div class="source-fragment">${html}</div></section>${type==='asbab'?`<div class="wahidi-actions"><a class="wahidi-primary" href="${openSource(s.s,v.a,'asbab')}" target="_blank" rel="noopener noreferrer">📜 فتح المادة في Quranpedia</a><a class="wahidi-secondary" href="${QP_WEB}/book/${WAHIDI_BOOK}" target="_blank" rel="noopener noreferrer">📚 كتاب أسباب النزول للواحدي</a></div>`:sourceLinks(s.s,v.a)}`;return}
    box.innerHTML=`<section class="ayah-detail"><h4>${type==='meanings'?'🔎 معاني الكلمات':'🕊️ سبب النزول'}</h4><p>تعذر جلب المادة الآن. عند توفر الإنترنت ستتم مزامنتها تلقائيًا، وأي مادة سبق فتحها ستظل محفوظة على جهازك.</p></section>${sourceLinks(s.s,v.a)}`;
  }
