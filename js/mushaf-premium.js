(function(){
  ...
  function renderTajweed(box,data,v){
    ...
    const hasDagger=/ٰ/.test(String(data.html||''));
    const daggerNote=hasDagger?`<div class="tajweed-inline-note"><b>علامة ألف خنجرية (ٰ)</b><span>العلامة فوق الحرف في النص العثماني تُقرأ ألفًا في موضعها. اضغط على العلامة نفسها لعرض الشرح.</span></div>`:'';
    box.innerHTML=`<div class="mushaf-note">التجويد المعروض هنا مأخوذ من نص تجويد مُعلَّم من المصدر المرجعي. لا نُنشئ حكمًا بالتخمين.</div>${daggerNote}<section class="connected-pronunciation"><div class="connected-pronunciation-head"><h4>🗣️ النطق عند الوصل</h4><span>تمثيل تعليمي فقط عند وجود إدغام معلَّم صراحةً في المصدر</span></div><div class="connected-pronunciation-text">${esc(connected.text)}</div>${joins?`<ul class="connected-joins">${joins}</ul>`:'<p class="connected-empty">لا توجد مواضع وصل كتابي مدمجة معلَّمة صراحةً في هذه الآية.</p>'}<div class="connected-disclaimer">التمثيل الكتابي لا يغني عن السماع من قارئ متقن، ولا يغيّر نص الآية الأصلي.</div></section><div class="ayah-tajweed-text" tabindex="0">${data.html}</div><div class="ayah-taj-source">المصدر: ${esc(data.source)}</div><div id="tajInspector" class="taj-inspector"><b>اضغط على الحكم الملوّن</b><p>سيظهر اسم الحكم وشرحه المختصر.</p></div>`;
    const els=box.querySelectorAll('tajweed[class]');
    els.forEach(el=>{
      const cls=normalizeClass(el),hasD=/ٰ/.test(el.textContent||'');
      if(hasD){el.setAttribute('title','ألف خنجرية (ٰ) — اضغط لعرض الشرح');el.setAttribute('aria-label','ألف خنجرية');el.classList.add('tajweed-clickable');}
      el.addEventListener('mousedown',e=>{e.preventDefault()},{passive:false});
      el.addEventListener('click',e=>{
        e.preventDefault();e.stopPropagation();
        els.forEach(x=>x.classList.remove('selected'));el.classList.add('selected');
        const info=hasD?['ألف خنجرية (ٰ)','علامة من علامات الرسم العثماني تُقرأ ألفًا في موضعها. وهي ليست حرفًا زائدًا يُضاف إلى نص المصحف.']:tajInfo(cls),ins=box.querySelector('#tajInspector');
        if(ins)ins.innerHTML=`<b>${esc(info[0])}</b><p>${esc(info[1])}</p><small>الوسم المصدر: ${esc(cls||'tajweed')}</small>`;
      },{passive:false});
    });
  }
  ...
  async function renderBook(box,s,a,book,title){
    ...
    if(!data){
      ...
      box.innerHTML=`<section class="ayah-detail empty-source-card"><h4>${esc(title)}</h4><p>جارٍ فتح المادة الأصلية داخل رفيق القرآن…</p><iframe class="quranpedia-study-fallback" title="${esc(title)}" loading="lazy" src="${openSource(s,a,type,book)}"></iframe><div class="source-badges"><a href="${openSource(s,a,type,book)}" target="_blank" rel="noopener noreferrer">فتح المصدر في Quranpedia</a></div></section>`;
      return;
    }
    const paragraphs=String(data.text||'').split(/\n{2,}/).map(x=>x.trim()).filter(Boolean).map(x=>`<p>${esc(x)}</p>`).join('');
    box.innerHTML=`<section class="ayah-detail quranpedia-direct-card"><div class="source-kicker">${esc(title)}</div><div class="source-content">${paragraphs||'<p>—</p>'}</div><div class="quranpedia-source-note">المصدر: Quranpedia · ${esc(data.book?.name||CONTENT.BOOKS?.[book]?.name||'الموسوعة القرآنية')}</div>${sourceLinks(s,a)}</section>`;
  }
