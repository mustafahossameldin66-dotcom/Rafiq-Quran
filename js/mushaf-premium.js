(function(){
  ...
  async function findBook(term,preferredRegex){
    const rows=extractBookRows(await qpBooksSearch(term));
    const norm=rows.map(x=>x?.book_info?{...x.book_info,_raw:x}:x).filter(Boolean);
    return norm.find(x=>preferredRegex.test(String(x.name||'')))||
           norm.find(x=>/غريب|أسباب|نزول|الواحدي/i.test(String(x.name||'')))||
           norm[0]||null;
  }
  ...
  async function qpAsbab(s,a){
    const direct=await qpBookContent(s,a,{id:WAHIDI_BOOK,name:'أسباب النزول'});
    if(direct&&extractContent(direct))return direct;
    const book=await findBook('أسباب النزول',/الواحدي|أسباب النزول/i);
    return qpBookContent(s,a,book);
  }
  ...
  async function playFallbackAyah(s,a){
    try{
      const url=`https://everyayah.com/data/Husary_128kbps/${String(s).padStart(3,'0')}${String(a).padStart(3,'0')}.mp3`;
      window.__rafiqMushafFallbackAudio ||= new Audio();
      const au=window.__rafiqMushafFallbackAudio;
      au.src=url; au.preload='auto'; au.volume=1;
      await au.play();
      toast('بدأت التلاوة — الحصري');
    }catch(e){
      toast('تعذر تشغيل التلاوة الآن. تأكد من اتصال الإنترنت وحاول مرة أخرى.');
    }
  }
  async function playAyah(s,a){
    const api=window.RAFIQ_API;
    if(api?.ensureReciterAndPlay){
      try{
        await api.ensureReciterAndPlay(s,a);
        await new Promise(r=>setTimeout(r,700));
        if(window.isAudioPlaying)return;
      }catch{}
    }
    await playFallbackAyah(s,a);
  }
  async function loadCleanQuranData(){
    try{
      const r=await fetch('./quran-uthmani.json',{cache:'force-cache'});
      if(!r.ok)throw new Error('local quran missing');
      const data=await r.json();
      if(Array.isArray(data)&&data.length===114){
        quran=data.map(s=>({
          ...s,
          s:Number(s.s),
          count:Number(s.count||s.verses?.length||0),
          verses:Array.isArray(s.verses)?s.verses.map(v=>({...v,a:Number(v.a),global:Number(v.global||v.number||0),text:String(v.text||'')})):[],
        }));
        return true;
      }
    }catch{}
  ...
  async function init(){
    ...
    const start=async()=>{
      const ok=await loadCleanQuranData();
      if(!ok){
      ...
      setStatus('',false);renderIndex();renderSurah();return true;
    };
    ...
    window.addEventListener('rafiq-data-ready',start);
    await start();
  }
  init();
