(function(){
  ...
  const storageKey='rafiq-mushaf-state-v4';
  ...
  const cacheDbName='rafiq-quran-cache-v3';
  ...
  function extractBookRows(data){if(Array.isArray(data))return data;if(Array.isArray(data?.books))return data.books;if(Array.isArray(data?.results))return data.results;if(Array.isArray(data?.items))return data.items;return[]}
  ...
  function quranpediaFrame(s,a,type,title){
    const src=`${QP_WEB}/embed?surah=${Number(s)}&ayah=${Number(a)}&type=${encodeURIComponent(type)}&theme=dark&ayah_text=1`;
    return `<section class="ayah-detail quranpedia-panel"><div class="source-kicker">📚 ${esc(title)} · Quranpedia</div><iframe class="quranpedia-iframe" src="${src}" title="${esc(title)} للآية ${s}:${a}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-write"></iframe><div class="source-badges"><a href="${src}" target="_blank" rel="noopener noreferrer">↗ فتح المصدر في Quranpedia</a></div></section>`;
  }
  async function renderStudyBody(s,v){
    ...
    if(studyTab==='overview'){
      box.innerHTML=`<div class="ayah-detail-grid">
        <section class="ayah-detail"><h4>📖 التفسير</h4><p>التفسير يُعرض مباشرة من موسوعة Quranpedia للآية المحددة.</p><button type="button" class="action info" data-jump-study="tafsir">فتح التفسير</button></section>
        <section class="ayah-detail"><h4>🔎 معاني الكلمات</h4><p>معاني الكلمات والمادة اللغوية تُعرض من Quranpedia دون نسخ قاعدة ثابتة داخل التطبيق.</p><button type="button" class="action info" data-jump-study="meanings">فتح المعاني</button></section>
        <section class="ayah-detail wahidi-mini"><h4>🕊️ سبب النزول</h4><p>عند وجود مادة موثقة للآية تظهر من قسم أسباب النزول في Quranpedia، وإلا يوضح المصدر عدم توفرها.</p><button type="button" class="action info" data-jump-study="asbab">فتح سبب النزول</button></section>
        <section class="ayah-detail"><h4>🎙️ التجويد</h4><p>التجويد هنا هو محلل رفيق التعليمي الموجود في Reverted، حرفًا حرفًا، حتى لا نفقد الوظيفة الأصلية للمشروع.</p><button type="button" class="action info" data-jump-study="tajweed">دراسة التجويد</button></section>
      </div>${sourceLinks(s.s,v.a,'tafsir')}`;
      box.querySelectorAll('[data-jump-study]').forEach(b=>b.onclick=()=>{studyTab=b.dataset.jumpStudy;document.querySelectorAll('.ayah-study-tab').forEach(x=>x.classList.toggle('active',x.dataset.tab===studyTab));renderStudyBody(s,v)});
      return;
    }
    if(studyTab==='tafsir'){
      box.innerHTML=quranpediaFrame(s.s,v.a,'tafsir','التفسير');
      return;
    }
    if(studyTab==='meanings'){
      box.innerHTML=quranpediaFrame(s.s,v.a,'meanings','معاني الكلمات');
      return;
    }
    if(studyTab==='asbab'){
      box.innerHTML=quranpediaFrame(s.s,v.a,'asbab','سبب النزول')+`<div class="wahidi-actions"><a class="wahidi-primary" href="${QP_WEB}/search/${encodeURIComponent('أسباب النزول')}/books" target="_blank" rel="noopener noreferrer">📚 كتب أسباب النزول في Quranpedia</a></div>`;
      return;
    }
  }
  function renderTajweed(box,v){
    const surah=Number(v?.s||surahNo||1), ayah=Number(v?.a||1);
    const src=`${QP_WEB}/surah/1/${surah}/tajweed?ayah=${ayah}`;
    box.innerHTML=`<section class="ayah-detail quranpedia-panel tajweed-source-panel"><div class="source-kicker">🎙️ التجويد · مصحف التجويد الملون في Quranpedia</div><p>تم إيقاف التحليل التجريبي للحروف داخل رفيق؛ أحكام التجويد تُعرض من مصحف التجويد الموثوق في Quranpedia، مع تلوين المواضع داخل الآية نفسها.</p><iframe class="quranpedia-iframe tajweed-quranpedia-frame" src="${src}" title="أحكام التجويد من Quranpedia · ${esc(v?.text||'')}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe><div class="source-badges"><a href="${src}" target="_blank" rel="noopener noreferrer">↗ فتح مصحف التجويد في Quranpedia</a><a href="${QP_WEB}/tajweed-rules" target="_blank" rel="noopener noreferrer">📚 قواعد التجويد في Quranpedia</a></div></section>`;
  }
