(function(){
  ...
  async function renderStudyBody(s,v){
    ...
    if(studyTab==='overview'){
      const base=`https://quranpedia.net/embed?surah=${encodeURIComponent(s.s)}&ayah=${encodeURIComponent(v.a)}&theme=dark&ayah_text=1`;
      box.innerHTML=`<div class="ayah-detail-grid"><section class="ayah-detail"><h4>📖 التفسير</h4><p>التفسير من الموسوعة القرآنية، ويُعرض داخل رفيق القرآن.</p></section><section class="ayah-detail"><h4>🔎 معاني الكلمات</h4><p>معاني الكلمات المرتبطة بالآية من Quranpedia.</p></section><section class="ayah-detail wahidi-mini"><h4>🕊️ سبب النزول</h4><p>أسباب النزول المتاحة للآية، ومنها مادة الواحدي.</p></section></div><div class="source-links"><a class="action" href="${base}&type=tafsir" target="_blank" rel="noopener noreferrer">فتح التفسير</a><a class="action" href="${base}&type=meanings" target="_blank" rel="noopener noreferrer">فتح معاني الكلمات</a><a class="action" href="${base}&type=asbab" target="_blank" rel="noopener noreferrer">فتح أسباب النزول</a></div>`;
      return;
    }
    const type=studyTab==='tafsir'?'tafsir':studyTab==='meanings'?'meanings':'asbab';
    const src=`https://quranpedia.net/embed?surah=${encodeURIComponent(s.s)}&ayah=${encodeURIComponent(v.a)}&type=${type}&theme=dark&ayah_text=1`;
    box.innerHTML=`<section class="ayah-detail quranpedia-frame-wrap"><div class="source-kicker">${type==='tafsir'?'📖 التفسير':type==='meanings'?'🔎 معاني الكلمات':'🕊️ أسباب النزول'}</div><iframe class="quranpedia-frame" title="Quranpedia" src="${src}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe>${type==='asbab'?`<div class="wahidi-actions"><a class="action" href="${src}" target="_blank" rel="noopener noreferrer">📜 فتح أسباب النزول في Quranpedia</a><a class="action" href="https://quranpedia.net/book/242" target="_blank" rel="noopener noreferrer">📚 كتاب أسباب النزول للواحدي</a></div>`:''}</section>`;
  }
  ...
  function init(){
    ...
    const start=()=>{
      ...
      if(!Array.isArray(data)||data.length<114){
        tries++;
        setStatus('⏳ جاري تجهيز بيانات المصحف…',true);
    ...
    document.addEventListener('rafiq-data-ready',start);
    window.addEventListener('rafiq-quran-ready',start);
    start();
  }
  init();
})();
