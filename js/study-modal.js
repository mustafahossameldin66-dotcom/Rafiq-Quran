(function(){
  ...
  const qState={surah:1,ayah:1,tab:'tafsir'};
  const topicMap={التفسير:'tafsir',التجويد:'tajweed','أسباب النزول':'asbab',tafsir:'tafsir',tajweed:'tajweed',asbab:'asbab'};
  ...
  const openSourceLinks=(s,a)=>`<div class="source-badges"><a href="${qp(s,a,'tafsir')}" target="_blank" rel="noopener">📖 تفسير Quranpedia</a><a href="${qp(s,a,'meanings')}" target="_blank" rel="noopener">🔎 معاني الكلمات</a><a href="${qp(s,a,'asbab')}" target="_blank" rel="noopener">🕊️ أسباب النزول</a></div>`;
  ...
  async function getTafsir(){ return ''; }
  ...
  async function render(){const {s,v}=currentData();if(!s||!v)return;setTabs();body.innerHTML='<div class="study-info-card"><p>جارٍ تجهيز دراسة الآية…</p></div>';const [taf,words]=await Promise.all([getTafsir(qState.surah,qState.ayah),getWordsMeaning(qState.surah,qState.ayah)]);let inner='';if(qState.tab==='tafsir')inner=`${qpFrame(qState.surah,qState.ayah,'tafsir','تفسير الآية من Quranpedia')}${openSourceLinks(qState.surah,qState.ayah)}`;else if(qState.tab==='words')inner=`${qpFrame(qState.surah,qState.ayah,'meanings','معاني كلمات الآية من Quranpedia')}${openSourceLinks(qState.surah,qState.ayah)}`;else if(qState.tab==='asbab')inner=`${qpFrame(qState.surah,qState.ayah,'asbab','سبب نزول الآية من Quranpedia')}${openSourceLinks(qState.surah,qState.ayah)}`;else if(qState.tab==='tajweed')inner=renderTajweed(s,v);else inner=`<section class="ayah-detail"><h4>📖 التفسير الميسر</h4><p>${esc(taf)}</p></section>${openSourceLinks(qState.surah,qState.ayah)}`;body.innerHTML=inner;bindStudyClicks(s,v);}
