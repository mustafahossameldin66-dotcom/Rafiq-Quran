...
(() => {
  ...
  function injectUI(){
    ...
    if(view&&!$('#memCoreRoot')){
      ...
      root.innerHTML=`
        <div class="mem-core-head"><div><span class="mem-kicker">المحرك الأساسي · الحفظ والمراجعة</span><h2>اعرف ماذا تحفظ، وماذا تراجع، ومتى تعود إليه</h2><p>التثبيت الأول = 7 جلسات ناجحة. بعدها اختر مراجعة أسبوعية ثابتة أو تكرارًا متباعدًا يتكيف مع أدائك.</p></div><div class="mem-core-actions"><button class="btn primary" id="memStartSession" type="button">ابدأ جلسة اليوم</button><span class="pill" id="memTodayKey">—</span></div></div>
        <div class="mem-today-grid"><article class="mem-today-card new"><small>حفظ اليوم</small><strong id="memNewToday">—</strong><span id="memTomorrowNew">—</span></article><article class="mem-today-card"><small>التثبيت</small><strong id="memStabilizeToday">—</strong><span>7 جلسات ناجحة لكل مقطع</span></article><article class="mem-today-card review"><small>المراجعة</small><strong id="memReviewToday">—</strong><span id="memReviewTomorrow">—</span></article><article class="mem-today-card alert"><small>المتأخر</small><strong id="memBacklog">—</strong><span>لا نكدّس عليك المراجعة بلا داعٍ</span></article></div>
        <div class="mem-stat-row"><div><small>المحفوظ السابق</small><b id="memPriorCount">0 آية</b></div><div><small>مجموعات الحفظ</small><b id="memActiveGroups">0 مجموعة</b></div><div><small>مجموعات مستحقة</small><b id="memReviewGroups">0 مجموعة</b></div><div><small>نقاط تحتاج دعمًا</small><b id="memWeakCount">0 مجموعة</b></div></div>
        ...
        <section class="mem-panel"><div class="mem-panel-head"><div><h3>حفظت اليوم</h3><p>لو حفظت مقطعًا مختلفًا عن المقترح، حدده هنا. بعد التسجيل تبدأ دورة التثبيت 7 جلسات.</p></div></div><div class="mem-form-grid"><label>من سورة<select id="memNewStartSurah"></select></label><label>من آية<input id="memNewStartAyah" type="number" min="1" value="1"/></label><label>إلى سورة<select id="memNewEndSurah"></select></label><label>إلى آية<input id="memNewEndAyah" type="number" min="1" value="1"/></label></div><div class="mem-quick-actions"><button class="btn" id="memNewWholeSurah" type="button">سورة كاملة</button><button class="btn primary" id="memRecordNew" type="button">سجل أنني حفظت</button></div></section>
        <section class="mem-grid-2"><section class="mem-panel"><div class="mem-panel-head"><div><h3>إضافة محفوظ سابق</h3><p>أسرع طريقة: اختر السورة واضغط «سورة كاملة»، أو حدد البداية والنهاية.</p></div></div><div class="mem-form-grid"><label>من سورة<select id="memPriorStartSurah"></select></label><label>من آية<input id="memPriorStartAyah" type="number" min="1" value="1"/></label><label>إلى سورة<select id="memPriorEndSurah"></select></label><label>إلى آية<input id="memPriorEndAyah" type="number" min="1" value="1"/></label></div><div class="mem-quick-actions"><button class="btn" id="memWholeSurah" type="button">سورة كاملة</button><button class="btn primary" id="memAddPrior" type="button">إضافة إلى المحفوظ السابق</button></div><div class="mem-range-list" id="memPriorList"></div></section><section class="mem-panel"><div class="mem-panel-head"><div><h3>خطة الحفظ والمراجعة</h3><p>حدد مقدار الحفظ، النظام الذي سيبدأ بعد التثبيت، وحجم الحمل اليومي.</p></div></div><div class="mem-form-grid three"><label>آيات جديدة/يوم<input id="coreNewPerDay" type="number" min="1" value="10"/></label><label>حد المراجعة/يوم<input id="coreReviewPerDay" type="number" min="1" value="30"/></label><label>مدة الجلسة<input id="coreSessionMinutes" type="number" min="5" value="25"/></label><label>نظام المراجعة<select id="coreReviewMode"><option value="spaced">تكرار متباعد · FSRS-like</option><option value="weekly">كل 7 أيام</option></select></label><label>يوم المراجعة الأسبوعية<select id="coreWeeklyReviewDay"><option value="0">الأحد</option><option value="1">الاثنين</option><option value="2">الثلاثاء</option><option value="3">الأربعاء</option><option value="4">الخميس</option><option value="5">الجمعة</option><option value="6">السبت</option></select></label><label>بداية الحفظ الجديد<select id="coreStartSurah"></select></label><label>آية البداية<input id="coreStartAyah" type="number" min="1" value="1"/></label></div><button class="btn primary" id="coreSavePlan" type="button">حفظ الخطة</button><div class="mem-note">التقييم بعد التسميع هو الذي يحدد الموعد القادم. في نظام التكرار المتباعد لا يوجد «سهل = أسبوع» ثابت؛ المدة تُحسب من أداءك السابق.</div></section></section>
        <section class="mem-panel"><div class="mem-panel-head"><div><h3>الأسبوع القادم</h3><p>صورة واضحة لما سيظهر لك كل يوم.</p></div></div><div class="mem-week-grid" id="memWeekGrid"></div></section>
        <section class="mem-panel"><div class="mem-panel-head"><div><h3>ما سيحدث لاحقًا</h3><p>كل مجموعة جديدة تظهر عدد جلسات التثبيت المتبقية ثم موعد المراجعة التالي.</p></div></div><div class="mem-upcoming-list" id="memUpcomingList"></div></section>
        <section class="mem-panel"><div class="mem-panel-head"><div><h3>تثبيت إضافي ونقاط الضعف</h3><p>المقاطع التي تكرر فيها «صعب» أو «أعد التثبيت» تعود إلى هنا.</p></div></div><div class="mem-range-list" id="memWeakList"></div></section>`;
