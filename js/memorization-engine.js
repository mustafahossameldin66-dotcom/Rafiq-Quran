...
(() => {
  ...
  function injectUI(){
    ...
    root.innerHTML=`<div class="mem-core-head compact"><div><span class="mem-kicker">الحفظ والمراجعة</span><h2>من هنا تدير رحلتك ببساطة</h2><p>أضف ما حفظته من قبل، حدّد ما تريد حفظه الآن، واترك رفيق القرآن يوضح لك موعد المراجعة والتثبيت.</p></div><div class="mem-core-actions"><button class="btn primary" id="memStartSession" type="button">ابدأ جلسة اليوم</button></div></div>
      ...
      <section class="mem-panel plan-flow-card"><div class="plan-flow-head"><div><span class="mem-kicker">4 · متى تراجع الحفظ الجديد؟</span><h3>اختر أسلوب المراجعة الذي يناسبك</h3><p>بعد فترة التثبيت، إمّا تراجع بدورة ثابتة كل 7 أيام، أو يحدد رفيق القرآن الموعد بالتكرار المتباعد حسب أدائك.</p></div></div><div class="review-mode-grid"><label class="review-mode-card"><input id="planReviewModeWeekly" name="planReviewMode" type="radio" value="weekly"><span><b>كل 7 أيام</b><small>موعد ثابت وبسيط. مناسب لمنهجية أسبوعية واضحة.</small></span></label><label class="review-mode-card"><input id="planReviewModeSpaced" name="planReviewMode" type="radio" value="spaced"><span><b>تكرار متباعد ذكي</b><small>الموعد يتغير حسب سهولة المراجعة وصعوبتها.</small></span></label></div><div class="review-settings-row"><label>مدة التثبيت قبل الدخول في المراجعة<input id="planStabilizationDays" type="number" min="1" max="30" value="7"><small>عدد الجلسات المتتالية؛ الافتراضي 7.</small></label></div></section>

      <section class="mem-panel plan-flow-card"><div class="plan-flow-head"><div><span class="mem-kicker">5 · المراجعة الأسبوعية للمحفوظ السابق</span><h3>كيف نوزع محفوظك السابق؟</h3><p>هذه ليست مراجعة الحفظ الجديد. إنها طريقة توزيع المحفوظ الذي أضفته قبل استخدام رفيق القرآن على أيام الأسبوع.</p></div><button class="btn" id="saveWeeklyReview" type="button">حفظ التوزيع</button></div><div class="weekly-review-controls"><label>طريقة التوزيع<select id="weeklyReviewDistribution"><option value="smart">ذكي ومتوازن — موصى به</option><option value="ayahs">آيات متقاربة</option><option value="pages">صفحات متقاربة</option><option value="surahs">حسب السور قدر الإمكان</option></select></label><label class="plan-switch compact"><input id="weeklyReviewEnabled" type="checkbox" checked><span></span><b>مفعّل</b></label></div><div class="weekly-review-preview" id="weeklyReviewPreview"></div></section>
    ...
    const syncReviewMode=()=>{const mode=document.querySelector('input[name="planReviewMode"]:checked')?.value||data.plan.reviewMode||'spaced';data.plan.reviewMode=mode;save();render();renderHomeCore();toast(mode==='weekly'?'تم اختيار المراجعة كل 7 أيام.':'تم اختيار التكرار المتباعد الذكي.');};
    $('#planReviewModeWeekly')?.addEventListener('change',syncReviewMode);$('#planReviewModeSpaced')?.addEventListener('change',syncReviewMode);
